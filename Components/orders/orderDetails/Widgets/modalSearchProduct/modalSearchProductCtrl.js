app.controller('modalSearchProductCtrl', ['$rootScope','$scope','$uibModalInstance','services','obj' ,function ($rootScope, $scope, $uibModalInstance,services,obj) {
	
	
	$scope.currentPage = 1;
	$scope.pageSize = 10;

	$scope.showProductProps = false;

	$scope.ok = function() {

		if ($scope.showProductProps) {
			if (obj.type == "product_main_list") $scope.addOrderItem($scope.selectedProduct);
			else if (obj.type == "product_kapitel") $scope.addOrderItem($scope.selectedProduct,obj.products_array);
	...
	$scope.selectProduct = function(row){
		...
		$scope.showProductProps = true;
		$scope.selectedProduct.primary_key = row.primary_key;
		$scope.selectedProduct.title = row.name;
		console.dir($scope.selectedProduct);
}
	

	

	$scope.oogetProductsData = function(searchText){

			$scope.showProductProps = false;
			services.common.loading.products = true;
			$scope.getProducts = [];

			services.api({
				method:"GET",
				data:"",
				url:localStorage.origin+localStorage.api_version+'/product/filter/?q='+searchText
			}).then(function(result) {
				
				services.common.loading.products = false;

			if (result) {
				$scope.getProducts = result.payload;
...
$scope.getProductsData = function(searchText){
